...
function M.get(cp)
    ...
    local info = cp.brightBlue
    local hint = cp.brightCyan

    return {
        -- These groups are for the native LSP cliencp. Some other LSP clients may
        -- use these groups, or use their own. Consult your LSP client's
        -- documentation.
        LspReferenceText = { bg = cp.xgray4 }, -- used for highlighting "text" references
        LspReferenceRead = { bg = cp.xgray4 }, -- used for highlighting "read" references
        LspReferenceWrite = { bg = cp.xgray4 }, -- used for highlighting "write" references
        ...
        LspSignatureActiveParameter = { fg = cp.brightOrange },
        ...
        LspCodeLens = { fg = cp.xgray5 }, -- virtual text of the codelens
    }
end
